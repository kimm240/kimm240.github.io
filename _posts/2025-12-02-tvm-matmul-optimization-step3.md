...
- `sch.fuse(*loops[-2:])`: `sch.fuse`는 여러 개의 루프를 한 개의 루프로 연결합니다. 즉 해당 명령어는 `loops`의 마지막 2개의 loop를 한 개의 루프로 연결합니다.
...
하지만, [추가 실험]2048×2048에서는 +101% 향상했습니다. 여러 SM에서 동시에 접근하는 데이터의 working set이 L2 캐시에 안정적으로 유지되지 못해 캐시 미스가 증가합니다.
